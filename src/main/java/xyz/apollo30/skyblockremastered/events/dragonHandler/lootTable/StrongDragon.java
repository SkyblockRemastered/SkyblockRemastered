...
public class StrongDragon extends DragLootStructure {

    public StrongDragon(Player p, int w, int e) {
        plr = p;
        weight = w;
        placedEyes = e;
        f = "Strong";
    ...
    public void getItem() {
        ...
        if (weight >= 550 && chance < 10) {
            giveFrags(weight - 550, Fragments.STRONG_FRAGMENT);
            plr.getInventory().addItem(Stones.WARPED_STONE);
            broadcastWorld(plr, Helper.getRank(plr, true) + " &ehas obtained a &9Warped Stone&e!");
        } else if (weight >= 450) {
            if (chance < 0.3) {
                double dragChance = Math.random() * 100;
                giveFrags(weight - 450, Fragments.STRONG_FRAGMENT);
                if (dragChance < 20) {
            ...
            giveFrags(weight - 400, Fragments.STRONG_FRAGMENT);
            plr.getInventory().addItem(Armor.STRONG_DRAGON_CHESTPLATE);
            broadcastPiece(plr, "Chestplate", f);
        } else if (weight >= 350 && chance < 50) {
            giveFrags(weight - 350, Fragments.STRONG_FRAGMENT);
            plr.getInventory().addItem(Armor.STRONG_DRAGON_LEGGINGS);
            broadcastPiece(plr, "Leggings", f);
        } else if (weight >= 325 && chance < 50) {
            giveFrags(weight - 325, Fragments.STRONG_FRAGMENT);
            plr.getInventory().addItem(Armor.STRONG_DRAGON_HELMET);
            broadcastPiece(plr, "Helmet", f);
        } else if (weight >= 300 && chance < 50) {
            giveFrags(weight - 300, Fragments.STRONG_FRAGMENT);
            plr.getInventory().addItem(Armor.STRONG_DRAGON_BOOTS);
            broadcastPiece(plr, "Boots", f);
        } else giveFrags(weight, Fragments.STRONG_FRAGMENT);
    }
