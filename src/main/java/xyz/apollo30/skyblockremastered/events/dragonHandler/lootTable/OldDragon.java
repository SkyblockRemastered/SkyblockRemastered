...
public class OldDragon extends DragLootStructure {

    public OldDragon(Player p, int w, int e) {
        plr = p;
        weight = w;
        placedEyes = e;
        f = "Old";
    ...
    public void getItem() {
        ...
        if (weight >= 550 && chance < 10) {
            giveFrags(weight - 550, Fragments.OLD_FRAGMENT);
            plr.getInventory().addItem(Weapons.WALKING_CANE);
            broadcastWorld(plr, Helper.getRank(plr, true) + " &ehas obtained a &5Walking Cane&e!");
        } else if (weight >= 450) {
            if (chance < 0.3) {
                double dragChance = Math.random() * 100;
                giveFrags(weight - 450, Fragments.OLD_FRAGMENT);
                if (dragChance < 20) {
                ...
                giveFrags(weight, Fragments.OLD_FRAGMENT);
            ...
            giveFrags(weight - 400, Fragments.OLD_FRAGMENT);
            plr.getInventory().addItem(Armor.OLD_DRAGON_CHESTPLATE);
            broadcastPiece(plr, "Chestplate", f);
        } else if (weight >= 350 && chance < 50) {
            giveFrags(weight - 350, Fragments.OLD_FRAGMENT);
            plr.getInventory().addItem(Armor.OLD_DRAGON_LEGGINGS);
            broadcastPiece(plr, "Leggings", f);
        } else if (weight >= 325 && chance < 50) {
            giveFrags(weight - 325, Fragments.OLD_FRAGMENT);
            plr.getInventory().addItem(Armor.OLD_DRAGON_HELMET);
            broadcastPiece(plr, "Helmet", f);
        } else if (weight >= 300 && chance < 50) {
            giveFrags(weight - 300, Fragments.OLD_FRAGMENT);
            plr.getInventory().addItem(Armor.OLD_DRAGON_BOOTS);
            broadcastPiece(plr, "Boots", f);
        } else giveFrags(weight, Fragments.OLD_FRAGMENT);
    }
