...
public class ProtectorDragon extends DragLootStructure {
    ...
    public void getItem() {
        ...
        if (weight >= 550 && chance < 10) {
            plr.getInventory().addItem(plugin.miscs.RESISTANT_SCALE);
            Masterclass.broadcastWorld(plr, Helper.getRank(plr, true) + " &ehas obtained &5Resistant Scale&e!");
        } else if (weight >= 450) {
            if (chance < 0.3) {
            ...
            plr.getInventory().addItem(plugin.armor.PROTECTOR_DRAGON_CHESTPLATE);
            Masterclass.broadcastPiece(plr, "Chestplate", f);
        } else if (weight >= 350 && chance < 50) {
            plr.getInventory().addItem(plugin.armor.PROTECTOR_DRAGON_LEGGINGS);
            Masterclass.broadcastPiece(plr, "Leggings", f);
        } else if (weight >= 325 && chance < 50) {
            plr.getInventory().addItem(plugin.armor.PROTECTOR_DRAGON_HELMET);
            Masterclass.broadcastPiece(plr, "Helmet", f);
        } else if (weight >= 300 && chance < 50) {
            plr.getInventory().addItem(plugin.armor.PROTECTOR_DRAGON_BOOTS);
