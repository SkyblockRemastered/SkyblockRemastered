...
public class OldDragon extends Dragon {
    ...
    public void getItem() {
        ...
        if (weight <=0 || damage <= 0) { Masterclass.broadcastWorld(plr, Helper.getRank(plr) + " has 0 weight L");
            ...
            Masterclass.broadcastWorld(plr, Helper.getRank(plr) + " &ehas obtained &5Walking Cane&e!");
            plr.getInventory().addItem(m.WALKING_CANE);
        } else if (weight >= 450) {
            // edrag chance
            if (chance < 0.3) {
                // got edrag
