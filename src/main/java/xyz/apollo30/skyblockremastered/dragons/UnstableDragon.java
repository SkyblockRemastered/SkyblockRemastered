...
public class UnstableDragon extends Dragon {
    ...
    public void getItem() {
        ...
        } else if (weight >= 550 && chance < 5) {
            Masterclass.broadcastWorld(plr, Helper.getRank(plr) + " &ehas obtained &6Lightning Staffe&e!");
            plr.getInventory().addItem(plugin.weapons.LIGHTING_STAFF);
        } else if (weight >= 450) {
            // edrag chance
            if (chance < 0.3) {
                // got edrag
