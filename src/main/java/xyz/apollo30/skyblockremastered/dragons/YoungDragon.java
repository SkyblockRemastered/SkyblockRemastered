...
public class YoungDragon extends Dragon {
    ...
    public void getItem() {
        ...
        if (weight <=0 || damage <= 0) { Masterclass.broadcastWorld(plr, Helper.getRank(plr) + " has 0 weight L");
            ...
            Masterclass.broadcastWorld(plr, Helper.getRank(plr) + " &ehas obtained &9Young Crystal&e!");
            plr.getInventory().addItem(plugin.miscs.YOUNG_CRYSTAL);
        } else if (weight >= 450) {
            // edrag chance
            if (chance < 0.3) {
                // got edrag
