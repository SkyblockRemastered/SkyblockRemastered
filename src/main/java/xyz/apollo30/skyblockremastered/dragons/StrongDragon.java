...
public class StrongDragon extends Dragon {
    ...
    public void getItem() {
        ...
        if (weight <=0 || damage <= 0) { Masterclass.broadcastWorld(plr, Helper.getRank(plr) + " has 0 weight L");
            ...
            Masterclass.broadcastWorld(plr, Helper.getRank(plr) + " &ehas obtained &9Warped Stone&e!");
            plr.getInventory().addItem(s.WARPED_STONE);
        } else if (weight >= 450) {
            // edrag chance
            if (chance < 0.3) {
                // got edrag
