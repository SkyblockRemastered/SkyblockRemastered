...
public class EntityDeath implements Listener {
    ...
    public void onEntityDeath(EntityDeathEvent e) {
        ...
        // Thousand Coins
        for (int i = 0; i < thousand_coins; i++) {
            e.getEntity().getWorld().dropItem(e.getEntity().getLocation(), thousand_coin);
        }

        // Hundred Coins
        for (int i = 0; i < hundred_coins; i++) {
            e.getEntity().getWorld().dropItem(e.getEntity().getLocation(), hundred_coin);
        }

        // Fifty Coins
        for (int i = 0; i < fifty_coins; i++) {
            e.getEntity().getWorld().dropItem(e.getEntity().getLocation(), fifty_coin);
        }

        // One Coins
        for (int i = 0; i < one_coins; i++) {
            e.getEntity().getWorld().dropItem(e.getEntity().getLocation(), one_coin);
        }
        ...
        if (e.getEntity().getPassenger().getCustomName().contains(Utils.chat("&cZealot"))) {
            ...
            if (Math.random() > random) {
                ...
                list.stream().filter(a -> e.getEntity().getKiller() != null && Math.abs(e.getEntity().getKiller().getLocation().getZ() + a.getZ()) <= 10 && Math.abs(e.getEntity().getKiller().getLocation().getX() + a.getX()) <= 10);
